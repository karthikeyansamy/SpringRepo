...
public class AdoInitiativeReportOrgWide {
    ...
    // Tunables
    private static final int INITIATIVE_PAGE_SIZE = 500; // keyset page size for initiatives
    private static final int INITIATIVE_BATCH_SIZE = 200; // how many initiatives to expand per recursive WIQL
    private static final int WORKITEM_BATCH_SIZE = 200; // workitemsbatch max
    private static final int SLEEP_MS_BETWEEN_HEAVY_CALLS = 200; // small pause to reduce throttling risk
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String project : projects) {
            ...
            while (true) {
                ...
                for (int i = 0; i < initiatives.size(); i += INITIATIVE_BATCH_SIZE) {
                    ...
                    // 4) Prepare report rows
                    for (Map.Entry<Integer, Set<Integer>> e : initiativeToChildren.entrySet()) {
                        int initiativeId = e.getKey();
                        for (int childId : e.getValue()) {
                            WorkItem child = details.get(childId);
                            if (child == null) continue; // skip missing
                            ReportRow row = new ReportRow();
                            row.projectName = child.projectName != null ? child.projectName : project;
                            row.areaPath = child.areaPath;
                            row.type = child.type;
                            row.initiativeId = initiativeId;
                            row.workItemId = child.id;
                            row.assignedTo = child.assignedTo;
                            row.createdBy = child.createdBy;
                            reportRows.add(row);
                        }
                    }
        ...
        try (FileWriter fw = new FileWriter("initiative_report_orgwide.csv", StandardCharsets.UTF_8)) {
            fw.write("ProjectName,AreaPath,WorkitemType,InitiativeID,WorkitemID,Assignee,Reporter\n");
            for (ReportRow r : reportRows) {
                fw.write(String.format("\"%s\",\"%s\",\"%s\",%d,%d,\"%s\",\"%s\"\n",
                        safe(r.projectName),
                        safe(r.areaPath),
                        safe(r.type),
                        r.initiativeId,
    ...
    private static List<Integer> fetchInitiativesPage(String project, int lastIdExclusive, int pageSize) throws Exception {
        // WIQL with keyset: System.Id > lastIdExclusive
        ...
        // WIQL may return many; limit to pageSize if needed
    ...
    private static Map<Integer, Set<Integer>> fetchHierarchiesForInitiatives(String project, List<Integer> initiativeIds) throws Exception {
        ...
        // Map sourceInitiative -> set of target ids (direct and recursive)
        ...
        // Ensure initiatives with no children get empty set (so we can include them later if desired)
        for (Integer id : initiativeIds) map.putIfAbsent(id, Collections.emptySet());

        return map;
    }

    // Overload without project (keeps earlier signature style)
    private static Map<Integer, Set<Integer>> fetchHierarchiesForInitiatives(List<Integer> initiativeIds) throws Exception {
        // Not used, kept for compatibility if needed
        return fetchHierarchiesForInitiatives("", initiativeIds);
    }
    ...
    private static Map<Integer, WorkItem> fetchWorkItemsBatch(Set<Integer> ids) throws Exception {
        ...
        for (int i = 0; i < idList.size(); i += WORKITEM_BATCH_SIZE) {
            ...
            for (JsonNode wi : arr) {
                ...
                // AssignedTo/CreatedBy can be either object or string; handle gracefully
